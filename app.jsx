...
// Lightweight chart fallbacks to keep the dashboard functional without a build step/dependencies
const ResponsiveContainer = ({ height=200, children }) => <div style={{ width:'100%', height }}>{children}</div>;
const BarChart = ({ children }) => <div style={{ width:'100%', height:'100%', background:'#f8fafc', border:'1px dashed #cbd5e1', borderRadius:8, padding:8 }}>{children}</div>;
const LineChart = ({ children }) => <div style={{ width:'100%', height:'100%', background:'#f8fafc', border:'1px dashed #cbd5e1', borderRadius:8, padding:8 }}>{children}</div>;
const Bar = () => null;
const Line = () => null;
const XAxis = () => null;
const YAxis = () => null;
    ...
    <nav style={{background:C.navy,padding:'0 24px',display:'flex',alignItems:'center',justifyContent:'space-between',height:58,position:'sticky',top:0,zIndex:100,boxShadow:'0 2px 12px rgba(0,0,0,0.3)'}}>
      ...
      <div style={{display:'flex',gap:4,alignItems:'center',flexWrap:'wrap'}}>
        ...
        {authState ? <>
          ...
          <div style={{width:32,height:32,borderRadius:'50%',background:grad,display:'flex',alignItems:'center',justifyContent:'center',color:'#fff',fontWeight:800,fontSize:14,cursor:'pointer',marginLeft:4}} onClick={()=>setAuthState(false)}>R</div>
        </> : <>
...
function AuthPage({ mode, nav, setAuthState }) {
  ...
  const handle = async () => {
    ...
    setAuthState(true); nav('dashboard'); setLoading(false);
  };
...
function AssessmentPage({ nav, setReport }) {
  ...
  const analyze = async () => {
    ...
    setLoadMsg('Analyzing your business profile…');
    try {
      await new Promise(r => setTimeout(r, 600));
      setLoadMsg('Matching against 200+ schemes and loan products…');
      await new Promise(r => setTimeout(r, 800));
      setLoadMsg('Calculating funding readiness score…');
      await new Promise(r => setTimeout(r, 600));
      const result = buildReport(form);

      setReport({
        ...result,
        stateSchemes: STATE_SCHEMES[form.state] || [],
        meta:{
          company: form.companyName || 'Your Company',
          state:   form.state,
          date:    new Date().toLocaleDateString('en-IN'),
          requestedAmt: form.loanAmount,
...
function App() {
  ...
  return (
    <div style={{fontFamily:'"Inter",system-ui,sans-serif',background:'#f8fafc',minHeight:'100vh'}}>
      ...
      {page==='signin'     && <AuthPage       mode="signin"  nav={nav} setAuthState={setAuthState}/>}
      {page==='signup'     && <AuthPage       mode="signup"  nav={nav} setAuthState={setAuthState}/>}
      {page==='assessment' && <AssessmentPage nav={nav} setReport={setReport}/>}
      {page==='report'     && <ReportPage     report={report} nav={nav}/>}
